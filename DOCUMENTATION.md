...
$ opensheets tools     # the 25-tool MCP surface, see §5
list_workspace
...
`App/Flags.swift`'s `summary`, which lists exactly these ten. `PLAN.md` §11's "default off" is
...
- **`reveal_range` requests are consumed.** The app opens the file if it is not already open — through
  ...
  **re-checked against the live grants at the moment of acting**, not trusted because the server wrote
  ...
  so a request that could not be honoured is not reconsidered forever. The same request file, the
  same consumer and the same rules now also serve `open_in_app`'s takeover requests
  ([§5.6, Lifecycle](#lifecycle)), where sheet and range are optional and the app's **startup
  sweep** — which consumes requests younger than 90 seconds — is the reader when the app was not
  running when the request was written.
...
> **Verified live in part; the rest is model-tested.** The protocol is tested in process against
> the real types on both sides: `HandshakePublisherTests` writes a record with the publisher and
> reads it back with the real `AppHandshake.presence(for:)`, asserting every field survives and
> that a hash derived independently on each side still agrees; a request written by the real
> `AppHandshake.requestReveal` is parsed by the real `HandshakeRevealConsumer`, with the stale,
> ungranted, malformed and relative-path refusals each asserted, and the full loop exercised in
> process. And the loop **has now been driven live**, through `open_in_app`: over real JSON-RPC
> against the embedded binary, a running app fronted, opened the file, switched sheet and selected
> the requested range, and — separately, with the app confirmed not running — `/usr/bin/open`
> launched it from nothing and the startup sweep applied the request. In both flows the
> confirmation was the app's own published presence read back through `opensheets workspace` and
> `opensheets selection` (the same code `get_selection` runs), reporting exactly the sheet and
> range requested — so a live session *has* returned a real selection, generated by the app's live
> UI state. What remains: `reveal_range`'s own polite flow has not been separately driven live
> (it shares the consumer and the selection machinery, but the presence-guarded path itself is
> still model-tested only), and nobody has literally watched the pixels — a formality for the two
> driven flows, stated rather than rounded away.
...
question; Apple's on-device foundation model answers it, reading the sheet through six tools
(`read_cells`, `write_cells`, `find_cells`, `calculate`, `append_rows`, `transform_cells`) and
saying under each answer which ones it used — with an explicit "no cells changed" suffix on
read-only turns, so a narrated edit that never happened indicts itself. Ranges accept a column
header's name ("Revenue") as well as A1. `calculate` is the formula engine as a read-only calculator, and it earns its schema
...
The tool surface is six tools rather than the server's 25 because the on-device model is small —
...
- **`path` is required on 24 of the 25 tools.** Absolute, and inside a granted folder. The exception
...
- **`preview` is accepted by all 25 tools**, boolean, default `false`. A dry run: the edit is applied
...
The count is 25: `list_workspace` and `list_files` arrived with discovery, and `new_workbook`,
`delete_file` and `open_in_app` arrived with the file lifecycle. Verify it with
...
| **`set_format`** · write | `path`, `range` (req), `sheet?`, plus any of `numberFormat`, `bold`, `italic`, `underline`, `strikethrough`, `wrap`, `fontName`, `fontSize` (1…409), `fontColor`, `fillColor`, `align`, `verticalAlign`, `columnWidth` (0…4000 pt), `rowHeight` (0…2000 pt), `freezeRows` (0…1,048,576), `freezeColumns` (0…16,384) | `formatted N cells in …`. Only the fields you pass change. The freeze arguments are sheet-level — they pin the top N rows / left N columns of the target sheet, and `0` explicitly unfreezes. |
...
| **`restore`** · write, destructive | `path`, `id?` (default: newest) | `restored 01J… · 3 sheets, 4,182 cells` — and takes a snapshot first. A missing target is resurrected from the snapshot's bytes, which is what makes it the documented undo for `delete_file`. |
| **`reveal_range`** · read | `path`, `range` (req), `sheet?` | Asks the running app to scroll to and select the range, opening the file first if it is not already a tab. Best-effort by contract — the app re-checks the grant and decides whether to act, and if the app is not running nothing happens. The takeover counterpart is `open_in_app` below. See [§3.4](#34-in-the-app). |
...
The file itself, from creation to the Trash — plus the one tool that puts it on the user's screen.
All three live in `Sources/SheetMCP/Tools/FileLifecycleTools.swift`, and they share a discipline the
rest of the surface only needs implicitly: each one's **first act on `path` is the grant check**,
before any existence check, any snapshot, any request file, and — for `open_in_app` — before the
launcher. Creation and deletion are the two operations where "does the file exist" is part of the
answer, which is exactly the question an ungranted caller must not be able to ask.

| Tool | Arguments | Returns |
| --- | --- | --- |
| **`new_workbook`** · write | `path` (req; extension must be writable: `xlsx xlsm xltx csv tsv`), `sheets?` (array of names, xlsx-family only, ≤255) | `created <path> · N sheets (Name1, Name2, …)`. |
| **`delete_file`** · write, destructive | `path` (req) | `trashed <path> (recoverable from the Trash)` plus `undo: restore(path, "<id>")`. Preview reports byte and sheet counts instead. |
| **`open_in_app`** · read | `path` (req), `sheet?`, `range?` | `asked OpenSheets to open <path> [· sheet S] [· range R]`, and that the app fronts within a few seconds; the selection request expires in 90 seconds if it does not. |

**`new_workbook` never overwrites.** An existing file at `path` is a refusal — *"the file already
exists — new_workbook never overwrites; write_range edits the file that is there"* — and the file
is left byte-identical; `write_range` remains the only mutation path for existing bytes. Creation
is the one write where "exists" is an error rather than a snapshot, so no snapshot is taken —
there is nothing to lose. Sheet names are validated to Excel's rules (31 characters, none of
`[ ] : * ? / \`) and to case-insensitive uniqueness, which is the collapse Excel itself performs;
a `.csv` or `.tsv` refuses a `sheets` list outright, because the format holds exactly one sheet
and its name comes from the file name. The created file goes through the same writer every save
uses, so it is indistinguishable from a saved one.

**`delete_file` is double-recoverable, and never a hard delete.** The order is fixed: grant check,
then a snapshot of the file's bytes (reason `before delete_file`), then
`FileManager.trashItem` — bytes first, then the file, so there is never a moment where the only
copy is the one being removed. The result names both safety nets: the Trash, and the
`restore(path, id)` undo. `restore` used to refuse a missing target; it now resurrects one from
the snapshot's bytes before opening the session, precisely so that documented undo line is true.
Only files inside a granted folder, in a format the server reads, can be deleted — and if the app
has the file open, its tab shows the designed missing state and the result says so.

**`open_in_app` is the takeover; `reveal_range` stays polite.** The pair is deliberate.
`reveal_range` writes a request only when the app is already publishing presence for the file, and
the app decides whether to act — it never launches anything, and against a closed app it honestly
reports that nothing was revealed. `open_in_app` writes the request unconditionally (new
`AppHandshake.requestOpen`, no presence guard — on a cold machine the reader is a process that
does not exist yet) and then launches or activates the app via
`/usr/bin/open -b com.quino.opensheets -- <path>`. `open` is idempotent: a running app is
activated and handed the file as a reopen event, a cold machine gets a launch, and the app's
startup sweep consumes requests younger than 90 seconds — which is exactly the window a
just-written one sits in. It is annotated read-only for the same reason `reveal_range` is: it
changes what is on screen, never any file. It is also the server's first and only subprocess,
scoped as [§9.3](#93-spreadsheet-content-is-untrusted-input) describes.

**Verified live, with one honest caveat about how.** Both `open_in_app` paths have been driven
end-to-end over real JSON-RPC against the embedded binary: a running app fronted, opened the file
as a tab, switched sheet and selected the requested range; and separately, with the app confirmed
not running, `/usr/bin/open` launched it from nothing and the startup sweep applied the request.
Confirmation came from the app's own published handshake presence read back through
`opensheets workspace` and `opensheets selection` — the selection reported matched the range
requested, in both flows. Nobody literally watched the pixels; since the presence record is
generated from the app's live UI state, that residue is a formality for these two flows, but it is
...
Write to a sheet that already exists, create a fresh workbook with the sheets you need
using new_workbook, or ask the user to add the sheet in Excel or the OpenSheets app and
then call this server again.
```

```
not supported in v0.1: deleting a sheet from an existing workbook
To clear its contents instead, call delete_rows over the sheet's used range. To start over
with a different sheet list, create a fresh workbook with new_workbook.
```

The `new_workbook` routing is not a contradiction of the refusal, and the distinction is worth
being precise about: the refusal is about *mutating an existing archive's* part structure in
place. Generating a fresh, complete package is what every save already does, so a workbook born
with exactly the sheets you need was never the risky operation — it just had no tool until the
lifecycle landed ([§5.6, Lifecycle](#lifecycle)).
...
**Every tool in [§5.6](#56-all-25-tools) is also a command.** That is a property the tests enforce
...
# Creating — never overwrites; sheet names are xlsx-family only
opensheets new budget.xlsx
opensheets new plan.xlsx Summary Data Q4
...
opensheets format budget.xlsx A1 freezeRows=1     # freeze the header; 0 unfreezes
...
# Deleting — snapshot first, then the Trash, so restore can undo it
opensheets delete-file old.xlsx

# The app, if it is running
opensheets selection budget.xlsx
opensheets reveal budget.xlsx C4:C20

# The app, whether or not it is running — launches it if needed
opensheets open budget.xlsx
opensheets open budget.xlsx B2:D9 --sheet Budget
...
`format` takes `set_format`'s arguments as `key=value` pairs, because there are sixteen of them and
sixteen flags would be worse (`freezeRows` and `freezeColumns` ride the same scheme, and being
integers they dodge the trap below):
...
- **The server spawns exactly one thing, deliberately.** Until the file lifecycle landed the honest
  ...
  `/usr/bin/open`, the bundle id is the constant `com.quino.opensheets`, the only variable argument
  is a path that has already passed the grant check — placed behind `--` so it cannot be read as an
  option — no shell is involved, and the child's stdin, stdout and stderr are routed to `/dev/null`
  explicitly (fd 1 is the protocol stream; `claimStdout` already redirected it, but the child does
  not get the chance to test that). Nothing from a cell reaches the argument list. The escape suite
  runs `open_in_app` against every hostile path like any other tool, and a spy-launcher test pins
  the part only a spy can see: a refused path never reaches the launcher and never writes a request
  file.
...
The recorded totals: **1,075 in-process grant checks — all 25 tools in `ToolRegistry.standard`
against all 43 escape cases, a number that grows with the surface — plus 147 against the shipped
binary, with zero escapes**, each refusal asserted to happen for the *right reason* rather than
merely to happen. For `open_in_app`, whose side effect is a subprocess, a spy launcher additionally
asserts that a refused path never reaches the launcher and never writes a request file
([§9.3](#93-spreadsheet-content-is-untrusted-input)).
...
**The app↔agent handshake gate is now mostly closed — by `open_in_app`, not by eyes.** This gate
used to read "no window has been seen scrolling, and no live MCP session has been observed
returning a real selection". Both halves have since been driven live ([§3.4](#34-in-the-app)):
`open_in_app` over real JSON-RPC made a running app front, open the file, switch sheet and select
the requested range, and — with the app confirmed not running — launched it from nothing through
`/usr/bin/open`, the startup sweep applying the request. Confirmation in both flows was the app's
own published presence read back through `opensheets workspace` and `opensheets selection`,
matching the requested sheet and range exactly. What is still open, precisely: `reveal_range`'s
presence-guarded polite path has not been separately driven live (it shares the consumer, so this
is narrow), and no human has watched the window move — the presence record is generated from the
...
| **The handshake's app half was missing, and this document said otherwise** | `get_selection` and `reveal_range` shipped with only their server half built: `AppHandshake.publish(_:)` was public and documented as *"the app calls this"*, and nothing called it — its argument type had no public initialiser, so nothing outside `SheetMCP` **could**. For the whole of that period `get_selection` answered "not open" about files that were open, `reveal_range` wrote request files nobody read, and §3.4 and §5.6 of this document described both as working. The app half now exists ([§3.4](#34-in-the-app)), is model-tested in both directions, and the loop has since been driven live through `open_in_app`, confirmed by reading the app's own published presence back ([§12.1](#121-release-gates-that-are-genuinely-open)). Recorded here rather than quietly deleted, because a documentation claim that outran the code is the defect class this project most needs to keep visible. |
| **Snapshot keys changed spelling when their file was deleted (found and fixed)** | Snapshots were keyed through Foundation's `resolvingSymlinksInPath`, which is existence-dependent: it resolves an existing path and returns a missing one untouched. A file reached through a symlinked parent — every `/tmp` path is one — was therefore keyed under its resolved spelling at capture time and looked up under its literal spelling the moment it mattered most: `delete_file` printed a real undo id, and `restore` answered `snapshot.notFound`. **How it evaded the suite:** the tests' scratch directories carry no symlinked prefix, so the two spellings always agreed there; one live `delete_file`/`restore` round against `/private/tmp` caught it on the first try. Keys now go through the same existence-independent `PathCanonicalizer` the grant boundary uses (`SnapshotStore.canonicalPath` — one identity rule for both), with a regression test asserting the key survives the file it names. One edge stated rather than hidden: snapshots taken *before* the fix, under a symlinked-prefix path's old resolved spelling, are looked up under the canonicaliser's spelling and will not be found — for the common `/Users/…` case the spellings are identical, so there is no practical loss. |
...
bytes, harmless, and it makes a naive `find Tests -name '*.swift' | wc -l` report 132 instead of
131 (re-counted while writing this revision; earlier revisions of this note carried the counts of a
smaller suite).
...
- 1,720 tests in 152 suites green, zero warnings, strict concurrency throughout.
- Grant enforcement: **1,075** in-process checks — every one of the 25 tools in
  `ToolRegistry.standard` against all 43 escape cases, so the number grows with the surface rather
  than with a list somebody maintains — plus 147 against the shipped binary, **zero escapes**, each
  refusal asserted to happen for the right reason, with positive controls so a
  vacuously-permissive bug fails loudly.
