...
| `formats/dates-1904.xlsx` | The 1904 epoch, same wall-clock dates (serials are exactly −1462). The two files **must render identically**. |
...
| --- | --- | --- |
| `hostile/zip-bomb.xlsx` | `zip.compressionRatioExceeded` | `sheet1.xml` inflates ~1030:1 to 200 MB. The ratio cap must fire **during** inflation. |
...
| `hostile/xxe-external-entity.xlsx` | `xml.dtdForbidden` | External entities pointing at `/etc/passwd` and at localhost. Neither may be resolved. |
| `hostile/billion-laughs.xlsx` | `xml.dtdForbidden` | Nine levels of 10× entity expansion — `&lol9;` is 10⁹ characters. |
| `hostile/dtd-doctype.xlsx` | `xml.dtdForbidden` | A harmless DOCTYPE. The policy must be blanket, not a heuristic that greps for `SYSTEM`. |
| `hostile/path-traversal.xlsx` | `zip.entryNameTraversal` | `../../../../etc/passwd` and the Windows `\..\` form as entry names. |
| `hostile/absolute-path-entry.xlsx` | `zip.entryNameAbsolute` | `/etc/passwd` and `C:\Windows\…` as entry names. |
| `hostile/nul-in-entry-name.xlsx` | `zip.entryNameIllegalCharacter` | An embedded NUL so a C-string API sees a different name than the zip index does. |
| `hostile/duplicate-entries.xlsx` | `zip.duplicateEntryName` | Two `sheet1.xml` entries with different content. Whichever you pick, another tool picks the other. |
| `hostile/truncated.xlsx` | `zip.truncatedArchive` | The first 55% of a valid xlsx — what a file caught mid-write looks like (§9). Must be a clean, retryable error. |
| `hostile/not-a-zip.xlsx` | `zip.notAnArchive` | A PDF renamed `.xlsx`. Trust the magic bytes, never the extension. |
| `hostile/encrypted.xlsx` | `xlsx.encryptedWorkbook` | An OLE2/CFB container advertising `EncryptedPackage` — must read as "password protected", not "corrupt". |
| `hostile/entry-count-bomb.xlsx` | `zip.entryCountExceeded` | 12,004 entries. Enforce while reading the central directory, not after building the table. |
| `hostile/lying-uncompressed-size.xlsx` | `zip.declaredSizeExceeded` | A ~120-byte entry whose ZIP64 header declares 10 GB. Reject on the declared size *before* allocating, then verify the actual length. |
...
| `hostile/unsupported-compression-method.xlsx` | `zip.unsupportedCompressionMethod` | Method 99 (AES). Only store (0) and deflate (8) occur in OOXML. |
| `hostile/dimension-4-billion-rows.xlsx` | `xlsx.dimensionExceedsLimits` | `<dimension>` claims 2³² rows and a row sits at `r=4294967295` — which also overflows a signed 32-bit parse to −1. |
| `hostile/deep-nesting-100k.xlsx` | `xml.depthLimitExceeded` | 100,000 levels of nesting: stack overflow for a recursive parser, 100k nodes for a DOM one. |
| `hostile/cell-40k-chars.xlsx` | `xlsx.cellTextTooLong` | 40,000 characters, past Excel's 32,767 ceiling. Accepted twin: `structure/long-cell-32k.xlsx`. |
| `hostile/nul-bytes-in-string.xlsx` | `xml.illegalCharacter` | Raw NUL, BEL and vertical tab inside `<t>` — all illegal in XML 1.0 — plus the legal `_x0000_` escaped form. |
| `hostile/missing-workbook-part.xlsx` | `xlsx.missingRequiredPart` | A valid ZIP whose rels target a `xl/workbook.xml` that is not there. |
| `hostile/invalid-cell-reference.xlsx` | `xlsx.invalidCellReference` | `r="0"`, `r=""`, a 5-letter column past XFD, a column with no row, a row with no column, a negative row. |
...
| `hostile/expected-errors.json` | — | The machine-readable version of this table: `expectedError`, `alsoAcceptable`, `mustNotHappen`, and the limit each case exercises. |
...
it is the recalculation engine that supplies the cached values. Everything else is generated
